...
export function rectBounds(halfX: number, halfZ: number): Bounds {
...
function createLotTexture(stalls: Stall[]): THREE.CanvasTexture {
  ...
  // Asphalt base with a little mottling
  ...
  // Stall outlines (open toward the lane)
  ctx.strokeStyle = '#e8edf2';
  ctx.lineWidth = 0.12 * sx;
  for (const s of stalls) {
    const halfW = (STALL_WIDTH / 2) * sx;
    const x = px(s.cx);
    const near = py(s.cz - s.row * (STALL_DEPTH / 2)); // lane side
    const far = py(s.cz + s.row * (STALL_DEPTH / 2)); // back of stall
    ctx.beginPath();
...
/** Builds the static parking-lot scenery. Returns a group to add to the scene. */
export function createParkingLot(stalls: Stall[]): THREE.Group {
  const group = new THREE.Group();

  // Wide dark ground beneath everything
  ...
  // Snowbanks around the perimeter
  ...
  // Arena building backdrop on the far side
  ...
  // Light poles at the four corners
  ...
  // Ambient + a soft fill so nothing is pitch black
